...
- **Dashboard tabs** — Overview (charts with savings rate overlay, burn rate, spending forecast, year-over-year, calendar) · Insights (AI, health score, income stability, 50/30/20, savings trend, investment rate) · Budget (limits, goals, category breakdown, fixed commitments) · Transactions (search, filter, sort, recategorize) · Assets
...
- **Financial goals** — Named savings targets (e.g. "Tabungan Haji 2027") with amount + deadline, and spending habit goals (consecutive months under limit); progress tracked via auto (from statements), manual input, or linked asset/pocket; **AI Goal Instrument Advisor** recommends Indonesian financial instruments (Deposito, RDPU, ORI/Sukuk, RD Saham, etc.) based on time horizon and monthly surplus; plans saved per-goal in vault
...
- **Vehicle & Property assets** — depreciation/appreciation model with default rates per subtype; current value auto-fills from model as you type; excluded from liquid calculations
- **Liabilities** — track KPR, KKB, KTA, credit card, and other debt; remaining balance, paid-off progress, monthly installment, maturity date; net worth = assets − liabilities
- **Multi-currency assets** — savings, gold, and investment assets support USD, EUR, SGD, and 7 other currencies; one-tap rate fetch from frankfurter.app (free, ECB-backed); in-memory cache avoids redundant fetches; stale rate indicator after 7 days
