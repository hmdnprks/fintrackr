...
- **Net worth summary card** — total value across all assets minus liabilities; when liabilities exist, shows an Assets − Liabilities = Net Worth row; allocation breakdown bars use total assets as base; shows month-over-month growth (↑/↓ amount and %) compared to the most recent snapshot ≥25 days old; hint shown until first snapshot exists; **net worth trend chart** — inline SVG area chart below the breakdown bars, groups daily snapshots by month, shows up to 12 months with indigo fill + line, y-axis labels (rb/jt/M format), x-axis month labels; visible when ≥2 months of data exist
- **Net worth & asset history** — snapshots recorded automatically whenever any asset value is saved or updated; stores both aggregate net worth (up to 365 daily entries) and per-asset value history (up to 365 daily entries per asset); each asset card shows ↑/↓ growth vs previous snapshot and a **mini sparkline** (last 6 months, green ↑ / red ↓, compact SVG line between value and type details); history included in JSON backup
- **Liquidity metrics** — displayed side-by-side on desktop, stacked on mobile:
  - *Stale asset indicator* — when any asset value hasn't been updated in 30+ days: dismissable amber banner at the top of the Assets section listing stale asset names; amber "Xd old" badge on each stale card; amber "Updated: …" timestamp with "· update recommended"; amber ⚠ warning on the net worth summary last-updated label
...
- **Asset cards** grouped by type with Heroicons, each showing current value, per-asset growth since last snapshot, institution, last-updated timestamp, and type-specific details:
  ...
  - Vehicle: subtype (Car / Motorcycle), purchase year + price, annual depreciation rate, estimated value today vs current value
  - Property: subtype (House / Apartment / Land), purchase year + price, annual appreciation rate, estimated value today vs current value
- **Vehicle & Property asset types** — depreciating/appreciating physical assets; depreciation model: `purchasePrice × (1 + rate/100)^years`; default rates: Car −10%/yr, Motorcycle −15%/yr, House +3%/yr, Apartment +2%/yr, Land +5%/yr; current value auto-fills live as user types purchase price/year/rate; manual override stops auto-fill; "Reset to estimate (Rp X)" link appears when overridden; current value field positioned below purchase fields in the form; excluded from emergency fund and liquid coverage (illiquid); amber note in modal explains exclusion
- **Liabilities** — separate section below asset cards for tracking debt; types: KPR (mortgage), KKB (vehicle loan), KTA (personal loan), Credit Card, Other; each entry records original amount, remaining balance, monthly installment, interest rate (% p.a.), start date, maturity/end date, optional link to a vehicle/property asset (shows equity context); card displays type badge, remaining balance in red, paid-off progress bar (% of original repaid), monthly installment + rate + months left; "Add Liability" button in section header; edit + delete per card; net worth = total assets − total liabilities; included in backup
- **Multi-currency assets** — savings, gold, and investment assets can be denominated in a foreign currency (USD, EUR, SGD, GBP, AUD, JPY, MYR, CNY, SAR, HKD); pocket, vehicle, property, and other types use IDR only; enter foreign amount + exchange rate; IDR equivalent auto-computed; **Fetch latest rate** button hits frankfurter.app (free, no API key, ECB-backed, daily updates) and fills the rate live; in-memory rate cache avoids redundant fetches when switching currency pills; IDR equivalent preview shown inline; rate age shown on card — turns amber with "tap edit to refresh" when >7 days old; `currentValue` always stored in IDR so all calculations (net worth, emergency fund, liquid coverage) work unchanged
...
- **Progress tracking** — pencil button on each card opens "Track Progress" modal with three modes:
  - **Auto** (default) — cumulative net (income − expense) from imported statements between start and deadline
  - **Manual** — enter the saved amount yourself; update any time
  - **Linked to asset** — reads the current balance from a saved savings/pocket/investment/gold asset; updates automatically when you update the asset
  - Mode badge shown on card: blue "Manual" or indigo "🔗 Asset Name" when not in auto mode
...
- **AI Goal Instrument Advisor** — indigo "AI Plan" button on active (non-complete, non-overdue) cards; opens a modal with:
  ...
  - Context summary: target, deadline, months left, monthly surplus (avg last 6 months income − expense), required/month, achievability badge (green ✓ / amber ⚠ / red ✗)
  ...
  - Monthly contribution chip, risk level badge (low/moderate/high), risk note, and summary
  - Last 3 plans per goal saved to vault; reloadable without re-running AI
