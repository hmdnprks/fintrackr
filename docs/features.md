...
- **Vehicle & Property asset types** — depreciating/appreciating physical assets; depreciation model: `purchasePrice × (1 + rate/100)^years`; default rates: Car −10%/yr, Motorcycle −15%/yr, House +3%/yr, Apartment +2%/yr, Land +5%/yr; "Use estimate" button fills current value with the computed figure; excluded from emergency fund and liquid coverage (illiquid); amber note in modal explains exclusion
