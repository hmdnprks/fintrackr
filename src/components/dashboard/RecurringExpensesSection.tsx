  ...
  return (
    <div className="bg-white dark:bg-gray-900 dark:border dark:border-gray-800 p-6 rounded-2xl shadow-sm">
      <div className="flex items-start justify-between mb-1">
        <div>
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Fixed Monthly Commitments</h2>
      ...
      <div className="space-y-2">
        {items.map((item, i) => (
          <div key={i} className="bg-gray-50 dark:bg-gray-800 rounded-xl px-4 py-3">
            ...
            <p className="text-sm font-medium text-gray-800 dark:text-gray-200 break-words mb-1.5">{item.description}</p>
            {/* Metadata row */}
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-400 dark:text-gray-500">{item.months} months</span>
              </div>
              <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 shrink-0">
                {formatIDR(item.avgMonthly)}<span className="text-xs text-gray-400 dark:text-gray-500 font-normal">/mo</span>
              </p>
            </div>
          </div>
        ))}
      ...
      <div className="mt-5 pt-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between text-sm">
        <span className="text-gray-500 dark:text-gray-400 font-medium">Total fixed per month</span>
