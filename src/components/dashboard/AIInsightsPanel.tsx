  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            {insights && !isLoading && (
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{period}</p>
            )}
          </div>
        </div>
      ...
      {isLoading ? (
        <div className="px-6 py-5 space-y-2.5">
          {[80, 65, 90, 55].map((w, i) => (
            <div key={i} className="flex items-start gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-gray-200 dark:bg-gray-700 mt-1.5 shrink-0" />
              <div className={`h-3.5 bg-gray-100 dark:bg-gray-800 rounded animate-pulse`} style={{ width: `${w}%` }} />
            </div>
          ))}
        </div>
      ) : bullets.length > 0 ? (
        ...
        <div className="px-6 py-5 text-sm text-gray-400 dark:text-gray-500">
          Generate insights for <span className="font-medium text-gray-500 dark:text-gray-400">{period}</span>.
